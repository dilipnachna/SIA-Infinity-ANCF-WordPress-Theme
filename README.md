...
**Status:** v0.4.0-alpha.1 Newsroom Frontend / Rank Smart read-only / staging-first
...
The current release does **not** automatically change URLs, redirects, canonicals, robots, schema, XML/News sitemaps, SEO titles/meta, publication state or article truth. Rank Math/Yoast/AIOSEO may remain active while Rank Smart observes evidence in read-only mode.
...
Rank Smart remains read-only in v0.4. It audits:
...
8. Assign the Primary Menu under WordPress menus if desired; otherwise the theme creates a category-based fallback navigation.
...
CI validates contracts, Rank Smart read-only safety, PHP syntax, WordPress component-version parity, installable ZIPs and the downloadable staging kit.
...
See `docs/NEWSROOM-FRONTEND-V0.4.md` and `docs/RANK-SMART-V0.3-READONLY.md` for the current boundaries.
