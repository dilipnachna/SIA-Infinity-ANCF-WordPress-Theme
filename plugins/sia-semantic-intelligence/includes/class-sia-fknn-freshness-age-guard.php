...
/**
 * Freshness-age guard for the More Stories surface.
 *
 * This is a late, read-only presentation policy. It does not change the base
 * Fibonacci-kNN graph, Related Stories, inlink recommendations, post content,
 * URLs or SEO output. Fresh targets prefer fresh sources, while older sources
 * can still survive when semantic/context evidence is exceptionally strong.
 */
final class SIA_FKNN_Freshness_Age_Guard {
    private const POLICY_VERSION = 'fknn-freshness-age-v1';
    ...
    public static function boot(): void {
        // Run after the surface bridge and final surface guard.
        add_filter('sia_ancf_news_more_stories_ids', [self::class, 'guard_more_stories'], 50, 4);
    }
    ...
    public static function guard_more_stories(array $ids, int $post_id, int $limit, array $exclude_ids = []): array {
        ...
        $fresh_target_days = max(1, min(365, (int) apply_filters('sia_fknn_fresh_target_days', 120, $target)));

        // Age pressure is a current/recent user-journey policy, not a global rule.
        // Archive/evergreen targets retain the existing semantic surface output.
        ...
        // Preserve currently approved candidates first, then allow the next
        // graph-backed neighbor to fill a rejected stale slot.
        ...
        foreach ($candidate_ids as $source_id) {
            ...
            $ranked[] = [
                'id' => $source_id,
                'score' => self::freshness_surface_score($recommendation, $target, $source, $age_days),
                'age_days' => $age_days,
            ];
        }
        ...
        return array_slice(array_map(static fn(array $row): int => (int) $row['id'], $ranked), 0, $limit);
    }
    ...
    private static function graph(WP_Post $target, int $limit): array {
        ...
        add_filter('sia_fknn_candidate_limit', [self::class, 'frontend_candidate_limit'], 1300, 2);
        $result = apply_filters('sia_fibonacci_knn_recommendations', [], $target, $limit);
        remove_filter('sia_fknn_candidate_limit', [self::class, 'frontend_candidate_limit'], 1300);
    ...
    public static function frontend_candidate_limit(int $limit, WP_Post $target): int {
        $configured = (int) apply_filters('sia_fknn_freshness_guard_candidate_limit', self::FRONTEND_CANDIDATE_LIMIT, $target);
    ...
    private static function passes_age_gate(array $recommendation, WP_Post $target, WP_Post $source, int $age_days): bool {
        ...
        $mild_days = max(30, min(365, (int) apply_filters('sia_fknn_more_age_mild_days', 120, $target)));
        $strong_days = max($mild_days + 1, min(730, (int) apply_filters('sia_fknn_more_age_strong_days', 365, $target)));
        $exceptional_days = max($strong_days + 1, min(3650, (int) apply_filters('sia_fknn_more_age_exceptional_days', 730, $target)));

        if ($age_days <= $mild_days) {
            $pass = true;
        } elseif ($age_days <= $strong_days) {
            $minimum = self::clamp01((float) apply_filters('sia_fknn_more_age_mild_minimum', 0.34, $target, $source));
            $evidence_floor = self::clamp01((float) apply_filters('sia_fknn_more_age_mild_evidence_floor', 0.30, $target, $source));
            $pass = $base >= $minimum
                && (($semantic !== null && $semantic >= $evidence_floor)
                    || ($context !== null && $context >= $evidence_floor)
                    || ($intent !== null && $intent >= $evidence_floor));
        } elseif ($age_days <= $exceptional_days) {
            $minimum = self::clamp01((float) apply_filters('sia_fknn_more_age_strong_minimum', 0.46, $target, $source));
            $semantic_floor = self::clamp01((float) apply_filters('sia_fknn_more_age_strong_semantic_floor', 0.36, $target, $source));
            $context_floor = self::clamp01((float) apply_filters('sia_fknn_more_age_strong_context_floor', 0.46, $target, $source));
            $intent_floor = self::clamp01((float) apply_filters('sia_fknn_more_age_strong_intent_floor', 0.28, $target, $source));
            $pass = $base >= $minimum
                && (($semantic !== null && $semantic >= $semantic_floor)
                    || ($context !== null && $context >= $context_floor))
                && ($intent === null || $intent >= $intent_floor);
        } else {
            // >730 days by default: exceptional evidence is required.
            $minimum = self::clamp01((float) apply_filters('sia_fknn_more_age_exceptional_minimum', 0.56, $target, $source));
            $semantic_floor = self::clamp01((float) apply_filters('sia_fknn_more_age_exceptional_semantic_floor', 0.46, $target, $source));
            $context_floor = self::clamp01((float) apply_filters('sia_fknn_more_age_exceptional_context_floor', 0.56, $target, $source));
            $intent_floor = self::clamp01((float) apply_filters('sia_fknn_more_age_exceptional_intent_floor', 0.34, $target, $source));
            $pass = $base >= $minimum
                && (($semantic !== null && $semantic >= $semantic_floor)
                    || ($context !== null && $context >= $context_floor))
                && ($intent === null || $intent >= $intent_floor);
    ...
    /** @param array<string,mixed> $recommendation */
    private static function freshness_surface_score(array $recommendation, WP_Post $target, WP_Post $source, int $age_days): float {
        $weights = ['base' => 8, 'temporal' => 5, 'semantic' => 3, 'intent' => 2, 'context' => 1];
        $values = [
            'base' => self::clamp01((float) ($recommendation['score'] ?? 0.0)),
        ...
        $decay = (float) apply_filters('sia_fknn_more_age_decay', $decay, $age_days, $recommendation, $target, $source);
        $score *= max(0.0, min(1.0, $decay));
